...
let {sendMessagecalculate} = require('queen-sithu-pro/sources/dc/cmd/cal')
let { MessageType, MessageOptions, Mimetype, GroupSettingChange, ChatModification } = require('@Sithuwa/queen-sithu-web');
let fs = require('fs');
let os = require('os');
...
QueenSithu['IntroduceCMD']({pattern: 'speedtest ?(.*)', fromMe: true, desc: DATA.SPEEDTEST_DESC, usage: 'speedtest user // speedtest server'}, (async (message, input) => {
...
QueenSithu['IntroduceCMD']({pattern: 'ping$', fromMe: true, delownsithucmd: false, desc: DATA.PING_DESC}, (async (message, input) => {
  var start = new Date().getTime();
  ...
  await message.client.sendMessage(
    ...
    QueenSithu['IntroduceCMD']({pattern: 'short ?(.*)', fromMe: WorkType, desc: DATA.URL}, (async (message, input) => {
    ...
    QueenSithu['IntroduceCMD']({pattern: 'cal ?(.*)', fromMe: WorkType, desc: 'Powerfull Calculater With >  - + * / × ÷ π e ( ) pie pi log lg ^ 1-9'}, (async (message, input) => {
       var textsithu = '';
if (message.reply_message) {
   textsithu = message.reply_message.text
} else {
    if (input[1] === '') return await message.client.sendMessage(message.jid,'*Need Any Input! Valid ==>  - + * / × ÷ π e ( ) pie pi log ^ 1-9*', MessageType.text);
   textsithu = input[1]
    }
        var calculate = await sendMessagecalculate(textsithu)
